...
func Build(m Moby, w io.Writer, pull bool, prefix string, tp string, decompressKernel bool, cacheDir string, dockerCache bool) error {
	if MobyDir == "" {
	...
	if m.Kernel.ref != nil {
		...
		err := ImageTar(m.Kernel.ref, prefix, kf, enforceContentTrust(m.Kernel.ref.String(), &m.Trust), pull, "", cacheDir, dockerCache, m.Architecture)
		if err != nil {
	...
	for _, ii := range m.initRefs {
		...
		err := ImageTar(ii, prefix, iw, enforceContentTrust(ii.String(), &m.Trust), pull, resolvconfSymlink, cacheDir, dockerCache, m.Architecture)
		if err != nil {
